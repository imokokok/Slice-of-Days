...
最新主体机制按用户确认的《主体机制-系统设计》接入：[日程与生活机制说明](docs/DAILY_LIFE_SYSTEM.md)。A 随身携带录音机，B 随身携带本子，物品栏和快捷键按角色分开。B 打开本子先看「今日日程」：实际时间、地点、事项、状态及下一项安排；顶部「今天的计划」仍可管理生活状态、已知活动、交通、班次及人物认识。计划不会自动执行；厨房工资在真实出餐后结算。现有美术、背景与五日故事保留。
...
A / D 走动；W 交谈；E 与物件互动；Space 继续对话；C 相机；A 角色按 R 录音，B 角色按 J 打开随身本和今日日程；G 相册；Tab 地图；B / I 随身物品；F 档案。Esc 退出当前对话或逐层收起。第五天见面后，通过日程页面选择角色，切换后随身物品同步更新。其他实际提示读取 Input Map。
