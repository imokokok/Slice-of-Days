...
func _test_edit_and_copy(first: Dictionary) -> void:
	...
	print("COPY_DIAGNOSTIC after click pubs=",publications.size()," records=",game.repository.load_recipes().size()," modal=",game._modal_kind)
	if publications.size()==2:
		print("COPY_DIAGNOSTIC status=",game._editor_status.text)
		game._save_recipe(true)
		print("COPY_DIAGNOSTIC after direct save pubs=",publications.size()," records=",game.repository.load_recipes().size())
	await _layout()
