...
static func run(g: Node2D) -> void:
	...
	var bad=load("res://extensions/collage_letter/scripts/finishing.gd").new(g)
	...
	var restored=load("res://extensions/collage_letter/scripts/finishing.gd").new(g);restored.restore(bad.serialize())
