...
func _ready() -> void:
	...
	cover=preload("res://extensions/collage_letter/scripts/desk_object.gd").new();cover.kind="folio";cover.font=g.font;cover.caption="";cover.size=size;cover.mouse_filter=Control.MOUSE_FILTER_IGNORE;add_child(cover)
var preview_id: int=-1
var paper_color:=Color("f8e1b5")
var edge_color:=Color("85baa7")
var postal_trim:=false
func _draw() -> void:
	if not material_book:preload("res://extensions/collage_letter/scripts/stationery_box.gd").paint(self,Rect2(Vector2.ZERO,size),true)
