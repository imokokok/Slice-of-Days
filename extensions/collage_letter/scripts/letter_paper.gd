...
static func texture(index: int) -> Texture2D:
	...
	if not textures.has(index):textures[index]=load("res://extensions/collage_letter/assets/open_pack/paper/"+FILES[index])
...
static func decoration(node: CanvasItem, rect: Rect2, style: int) -> void:
	if style==7:
		var leaf: Texture2D=load("res://extensions/collage_letter/assets/open_pack/icons/leaves.svg")
		for i in 4:
