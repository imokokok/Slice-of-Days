...
static func data(cycle:int=0) -> Dictionary:
	var source:Dictionary=JSON.parse_string(FileAccess.get_file_as_string("res://extensions/collage_letter/assets/sea_example.json"))
...
static func artwork(game, reply:bool) -> String:
	...
	var words=preload("res://extensions/collage_letter/scripts/letter_renderer.gd").new();view.add_child(words)
	if reply:
...
static func start_reply(game) -> void:
	...
	game.letter_data=preload("res://extensions/collage_letter/scripts/letter_data.gd").fresh();game.letter_data.sender="Solmere";game.letter_data.recipient=data(game.sample_cycle)["name_"+game.L.language]
...
static func populate_reply(game,parent:Node,sample:Dictionary)->void:
	...
	var note=game.Piece.new();note.source_id=-1;note.font=preload("res://extensions/collage_letter/scripts/letter_renderer.gd").HAND;note.handwriting=sample["tag_"+game.L.language];note.position=Vector2(698,726);note.rotation=-.045;note.scale=Vector2.ONE*.7
