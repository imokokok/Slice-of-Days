extends Control
@export_dir var image_folder := "res://extensions/observatory/assets/slideshow/"
...
func scan_images() -> void:
 ...
 files.sort()
 if files.is_empty():
  front.texture = load("res://extensions/observatory/assets/background/photo_placeholder.svg")
  front.visible = not preserve_background_when_empty
  back.hide()
  return
 front.show()
 back.show()
 var saved := files.find(ObservatoryState.slide_file)
 ObservatoryState.slide_index = saved if saved >= 0 else ObservatoryState.slide_index % files.size()
 front.texture = texture_at(ObservatoryState.slide_index)
func texture_at(index: int) -> Texture2D:
 ...
 return tex if tex else load("res://extensions/observatory/assets/background/photo_placeholder.svg")
func _process(delta: float) -> void:
 if files.size() < 2 or transitioning: return
 ObservatoryState.slide_elapsed += delta
 if ObservatoryState.slide_elapsed >= hold_seconds: advance()
func advance() -> void:
 transitioning = true
 var next := (ObservatoryState.slide_index + 1) % files.size()
 if random_order: next = (ObservatoryState.slide_index + randi_range(1, files.size()-1)) % files.size()
 ...
 ObservatoryState.slide_index = next
 ObservatoryState.slide_file = files[next]
 ObservatoryState.slide_elapsed = 0.0
 transitioning = false
