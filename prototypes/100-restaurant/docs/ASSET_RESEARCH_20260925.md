...
用户后续明确要求：Little Chef 素材采用时，必须在其基础上改造后再接入，以现有团队手绘食材为风格标准；其他外部补充素材同样执行。改造涵盖轮廓、笔触、配色、明暗、视角、比例及整件到切面的连续性，不能直接混用原包。具体检查项见 [美术方向](ART_DIRECTION.md)。本次仅同步接入规则，上表的下载、采用与许可状态没有因此改变。
