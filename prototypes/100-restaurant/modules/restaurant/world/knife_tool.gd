...
func _draw() -> void :
	...
	if cutting_guide:
		...
		draw_circle(BLADE_MID, 3.0, gold)
		draw_line(BLADE_MID + Vector2(0, 5), BLADE_MID + Vector2(0, 19), gold, 2.0, true)
		draw_colored_polygon(PackedVector2Array([BLADE_MID + Vector2(-5, 15), BLADE_MID + Vector2(5, 15), BLADE_MID + Vector2(0, 22)]), gold)
