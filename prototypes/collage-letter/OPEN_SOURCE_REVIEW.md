...
核验日期：2026-09-26。目标是保留拼贴玩法，改善联机稳定性、录音反馈和 2D 手绘工作台。许可文件随源码保存，不把许可说明放进玩家操作流程。
...
- 678 个材料全部经 Godot 渲染和裁剪，图案使用 240px SVG 固有尺寸，避免放大低分辨率导入贴图。
- 保留矩形/自由裁剪、纸片变换和折信；补充涂鸦、8 种胶带、独立字母、连续装信、火柴盒点火和实体信箱。
...
参考 [Sticky Business](https://store.steampowered.com/app/2303350/Sticky_Business/) 的拼贴排列与包装步骤，以及 [Tiny Glade](https://store.steampowered.com/app/2198150/Tiny_Glade/) 对细微操作反馈的重视。只借鉴交互原则，未复制这些商业游戏的代码或美术。

新增 CC0 原图逐项核验来源页；手绘信封采用 Kutejnikov Papers 的 casual 图集。桌面采用浅木纹配合平面色洗，工具使用曲线轮廓与纸纹笔触。物件保留各自的输入、进度、遮挡与声音，界面布局覆盖两种语言、三种窗口尺寸和五种工具状态。
