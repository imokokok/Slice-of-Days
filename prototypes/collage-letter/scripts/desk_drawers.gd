...
func _ready() -> void:
	...
	var controls=preload("res://scripts/writing_controls.gd").new();controls.g=g;add_child(controls)
	...
	if g.typewriter_open:
		var station=preload("res://scripts/typewriter_station.gd").new();station.name="TypewriterStation";station.g=g;station.desk=self;add_child(station)
...
func build_writing() -> void:
	writing=preload("res://scripts/letter_input.gd").new();writing.name="LetterWriting";writing.game=g;writing.renderer=g.letter_text_node
...
func finish_book_turn(old_leaf: ImageTexture, direction: int) -> void:
	...
	var leaf=preload("res://scripts/book_turn.gd").new();leaf.name="TurningLeaf";leaf.leaf=old_leaf;leaf.direction=direction;leaf.position=Vector2(25,12);leaf.size=Vector2(317,486)
