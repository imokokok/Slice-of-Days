...
func _ready() -> void:
	...
	status.text = LocalizationSystem.text("录音机\n记录此刻听见的声音")
	for i in 3:
		var b := preload("res://scripts/ui/components/solmere_button.gd").new(); b.variant="camera"; b.text=LocalizationSystem.text(["● 开始录音","标记","收起"][i]); b.position=Vector2(18 if i==0 else 224+(i-1)*92,214); b.size=Vector2(196 if i==0 else 86,48); card.add_child(b)
...
func _save(warning := "") -> void:
	...
	if item.is_empty(): status.text = LocalizationSystem.text(store.last_error+"\n"+SettingsSystem.binding_text("open_recorder")+" "+LocalizationSystem.text("重试保存")); record_button.text=LocalizationSystem.text("重试保存"); mark_button.disabled=true; close_after = false; return
