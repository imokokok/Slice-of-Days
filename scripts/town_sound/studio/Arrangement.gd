...
func load_pcm(sample_id: String) -> Dictionary:
	...
	for i in pcm.size():
		pcm[i] = float(bytes.decode_s16(i * 2 * channels)) / 32768.0
...
func save_project() -> bool:
	...
	var file := FileAccess.open("user://projects/current.json.tmp", FileAccess.WRITE)
	if file == null:
		error = "工程无法写入。"
		return false
	...
	if result != OK:
		error = "工程写入不完整。"
		return false
	result = DirAccess.rename_absolute("user://projects/current.json.tmp", "user://projects/current.json")
	if result != OK:
		error = "工程保存失败。"
	return result == OK

func load_project() -> bool:
	if not FileAccess.file_exists("user://projects/current.json"):
		return false
	var parser := JSON.new()
	if parser.parse(FileAccess.get_file_as_string("user://projects/current.json")) != OK or not parser.data is Dictionary:
		error = "工程损坏，保留原文件。"
		return false
	var value: Dictionary = parser.data
	if not value.get("clips") is Array or value.get("muted", []).size() != 4 or value.get("gains", []).size() != 4:
		error = "工程结构无效。"
		return false
