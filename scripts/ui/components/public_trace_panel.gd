...
func _ready() -> void:
	...
	var heading := Label.new(); heading.text=LocalizationSystem.text("起居角 · 唱片与纸张" if GameState.current_location in ["residence","dorm"] else "小镇的公共作品架"); heading.add_theme_font_size_override("font_size",28); rows.add_child(heading)
