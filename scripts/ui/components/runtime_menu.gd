...
func _ready() -> void:
	...
	if not settings:
		...
		for row in [["主音量","master_volume"],["音乐","music_volume"],["音效","sound_effects_volume"]]:
			var title := Label.new(); title.text=str(row[0]); title.add_theme_color_override("font_color",Color("fffaf0")); box.add_child(title)
	...
	var motto := Label.new(); motto.text="Same sky,\na different view."; motto.position=Vector2(1050,637); motto.add_theme_color_override("font_color",Color("fffaf0")); add_child(motto)
