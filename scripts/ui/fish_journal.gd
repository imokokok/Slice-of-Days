...
func _ready() -> void:
	...
	var close := preload("res://scripts/ui/components/solmere_button.gd").new(); close.text="返回 · "+SettingsSystem.binding_text("ui_cancel"); close.position=Vector2(1290,42); close.size=Vector2(230,48); close.pressed.connect(queue_free); add_child(close)
	...
	for caught: Dictionary in catches:
		...
		button.text="%s · %.1f 厘米\n第 %d 天 · %s" % [FISH.species(str(caught.id)).get("name",caught.get("name","鱼获")),float(caught.length_cm),int(caught.day),"带回厨房" if caught.kept else "放回海里"]
		button.pressed.connect(_select.bind(caught))
	if catches.is_empty():
		var empty := Label.new(); empty.text="还没有鱼获记录。\n钓到后，无论收下或放生，\n都可以在这里翻看。"; rows.add_child(empty)
		_words("先认识这里的海鱼",28)
		for species: Dictionary in FISH.SPECIES:
			var button := preload("res://scripts/ui/components/solmere_button.gd").new(); button.text=species.name+" · 资料"; button.custom_minimum_size.y=52; detail.add_child(button); button.pressed.connect(_select.bind({"id":species.id}))
	else: _select(catches[0])
	close.grab_focus()
func _words(text: String, point := 23) -> Label:
	var label := Label.new(); label.text=text; label.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART; label.add_theme_font_size_override("font_size",point); detail.add_child(label); return label
func _select(caught: Dictionary) -> void:
	...
	var source := LinkButton.new(); source.text="阅读鱼种资料"; source.add_theme_font_size_override("font_size",21)
