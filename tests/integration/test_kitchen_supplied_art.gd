...
func run() -> void:
	...
	check(not scene.prep_board.drag_stroke(Vector2(300,220),Vector2(370,230)) and scene.prep_board.strokes==0,"A drag missing the ingredient should not cut it")
	check(scene.prep_board.drag_stroke(Vector2(42,95),Vector2(122,137)) and scene.prep_board.strokes==1 and scene.prep_board.knife_angle>0.0,"A diagonal drag across the carrot should make one angled cut")
	...
	check(scene.prep_board.strokes==1,"Releasing after a drag should not count a duplicate cut")
	scene.prep_board.pressed.emit()
	check(scene.prepared_tokens.is_empty() and not scene.prep_board.cuts.has("carrot"),"A second cut should keep the carrot in progress")
	scene.prep_board.pressed.emit()
	...
	check(scene.instruction_label.position.y<scene.illustrated_pot.position.y and scene.selection_label.text.contains("实际下锅"),"Stage guidance should stay above the pot while actual additions appear in the tool area")
