...
func run() -> void:
	...
	check(preview.find_children("DossierAction_*","Button",true,false).is_empty(),"Dossier checklist no longer opens the duplicate generic form")
	check(preview.find_children("DossierTab_*","Button",true,false).is_empty(),"Illustrated side tabs remain part of the single dossier view")
	...
	check(shell.overlay.find_children("Starter_*","Button",true,false).is_empty(),"First collection does not reveal the duplicate form page")
	await shot("dossier")
	await close_paper(shell)
	shell._unhandled_input(key(KEY_F))
	await create_timer(.1).timeout
	...
	check(shell.overlay.find_children("DossierAction_*","Button",true,false).is_empty(),"Dossier rows have no links to the duplicate form")
	check(shell.overlay.find_children("DossierTab_*","Button",true,false).is_empty(),"Dossier tabs have no links to the duplicate form")
