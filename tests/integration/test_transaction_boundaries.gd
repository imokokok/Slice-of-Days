...
func run() -> void:
	...
	state.current_minute = 700 # B's free block ends at 12:00.
	...
	check(not bool(state.buy_item({"id":"soap","price":10,"minutes":21}).get("ok", false)), "Purchases cannot cross a reserved time block")
	...
	check(state.current_minute == 705 and int(state.daily_spending.get("1", 0)) == 10, "Purchase charges five minutes and records daily spending")
	...
	check(state.current_minute == 705 and int(state.daily_spending.get("1", 0)) == 10 and state.money_ledger.size() == start_ledger + 1, "Save data restores time and the expense exactly once")
