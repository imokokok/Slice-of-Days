...
func run():
	...
	var screen=load("res://scenes/town_sound/Recorder.tscn").instantiate(); screen.shop_mode=true; current_scene.add_child(screen); await settle()
	var old_count: int=screen.store.list_samples().size()
	check(not screen.record_button.disabled and screen.save_button.disabled,"Recorder exposes the available action, not an empty save")
	...
	check(screen.live_screen.frames_seen>0 and screen.waveform.peaks.size()>0,"Live window and waveform update while capturing")
	check(screen.source_picker.disabled,"Recording locks source selection")
	screen.set_compact(true); await settle(); check(not screen.page_scroll.visible and screen.compact_bar.visible,"Compact recording clears the workspace")
	screen.stop_button.pressed.emit(); await settle()
	check(screen.draft!=null and screen.draft.get_length()>.2,"Stop yields an actual playable WAV")
	check(not screen.save_button.disabled and not screen.compact,"Recorded draft restores the device and save action")
	screen.preview_button.pressed.emit(); await settle(); check(screen.player.playing,"Draft preview plays actual audio")
	screen.name_input.text="海风采样 · UI复查"; screen.save_button.pressed.emit(); await settle()
	check(screen.draft==null and screen.store.list_samples().size()==old_count+1,"Save places the recorded file in the actual library")
	var item: Dictionary=screen.store.list_samples().back(); check(not bool(item.missing),"Saved recording has a file")
	check(not screen._sample_memory_text(item).contains(gs.current_role+"采集"),"Recording provenance does not reveal the role letter before the story reveal")
	screen.request_close(); await settle(); check(get_nodes_in_group("town_sound_workspace").is_empty(),"Recorder closes without leaving a modal behind")
