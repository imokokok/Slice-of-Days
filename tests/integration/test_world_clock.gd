...
func run() -> void:
	...
	check(state.current_minute == 495, "60 real seconds must equal 15 game minutes")
	check(state.clock_text() == "08:15", "Clock must display hours and minutes only")
	state.advance_world_clock(3.5)
	check(state.current_minute == 495, "Partial minute must not change displayed time")
	var saved: Dictionary = state.to_save_data()
	state.begin_new_game()
	state.load_save_data(saved)
	state.advance_world_clock(0.5)
	check(state.current_minute == 496, "Save/load must preserve fractional clock progress")
	state.begin_new_game()
	for i in 3600: state.advance_world_clock(1.0 / 60.0)
	check(state.current_minute == 495, "Frame-sized ticks must retain the exact 15:1 rate")
