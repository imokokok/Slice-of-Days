...
func run() -> void:
	...
	check(shop.item_list.columns==3,"Goods on three-column shelf")
	...
	check(shop.status_label.text.contains("小票已收入生活记录"),"Receipt destination displayed")
	...
	var style: StyleBoxFlat = shop.get_node("ShopCounter").get_theme_stylebox("panel")
	check(style.shadow_size==0 and style.bg_color==Color("edf3f4"),"Clean shop surface with no shadow")
	await capture("shop")
	...
	if game.token_buttons.has("bread"):
		check(game.token_buttons.bread.get_theme_stylebox("normal").bg_color.a>0,"Selected ingredient retains yellow feedback")
