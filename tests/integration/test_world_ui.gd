...
func run() -> void:
	...
	check(shop.status_label.text.contains("小票已收好"),"Receipt displayed")
	...
	check(style.shadow_size==0 and style.bg_color==Color("faf7ee"),"Shared warm paper with no shadow")
	await capture("shop")
